...
class Volume:
    def __init__(self, PATH_SAVE=FOLDER_SAVE, wait_after_click=1) -> None:
        PATH_SAVE = os.path.abspath(PATH_SAVE)
        self.PATH_SAVE = PATH_SAVE
        self.folder_F0 = PATH_SAVE + "\\Volume\\MorningStar\\F0"
        self.sleep_time = wait_after_click
    ...
    def get_volume(self, symbol, driver: webdriver.Edge):
        ...
        try:
            ...
            button = driver.find_element(By.XPATH, '//*[@id="__layout"]/div/div/div[2]/div[3]/div/main/div/div/div[1]/section[1]/div/div[2]/div[1]/button[3]')
            button.click()
            time.sleep(self.sleep_time)

            soup = BeautifulSoup(driver.page_source, "html.parser")
            table = soup.find("ul", attrs={"class": "stock__quote-content stock__quote-content--trading-information"})
            li = table.find_all("li")[6]
            text = li.text
            if text.startswith("Shares Outstanding "):
                return True, text.split("Shares Outstanding ")[1]
    ...
    def get_all_volume(self, num_thread=8, MAX_TRIAL=5):
        print("===== Kéo volume MorningStar =====", flush=True)
        try:
